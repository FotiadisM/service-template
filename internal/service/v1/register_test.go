...
func (t *UnitTestSuit) TestRegister() {
	...
	store := mockStore.NewMockStore(t.T())
	store.EXPECT().CreateUser(mock.Anything, mock.Anything).RunAndReturn(func(ctx context.Context, params queries.CreateUserParams) (queries.User, error) {
		return queries.User{
			ID:        params.ID,
			Email:     params.Email,
